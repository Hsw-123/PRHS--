...
public class CouponInfoServiceImpl extends ServiceImpl<CouponInfoMapper, CouponInfo> implements CouponInfoService {
    ...
    public BigDecimal useCoupon(UseCouponForm useCouponForm) {
        ...
        if(reduceAmount.doubleValue()>0) {
            //更新coupon_info使用数量
            //根据id查询优惠卷对象
            CouponInfo updateCouponInfo = couponInfoMapper.selectById(couponInfo.getId());
            Integer useCount_old = updateCouponInfo.getUseCount();
            updateCouponInfo.setUseCount(useCount_old+1);
